...
hosting of DNS services in itself, ignoring any attacks that could take place
during transmission -- these are extensively documented in
\cite{weaknesses-dns}, \cite{rfc3833} and \cite{dns-bind-sec}.

Both threats and protective measures are extracted from \cite[sections 3, 5,
7, and 10]{secdns}. Some are omitted, as they are only pertinent to enterprise
environments, not to the service a top-level domain resolver would offer. 

Unfortunately, uniform statistical data concerning these threads and measures
is either scarce or nonexistent.

\todo{Some quantitative goal to make decisions}
\todo{Make up data}
\todo{Prioritise defensive measures}
