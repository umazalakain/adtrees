...
These cutsets can then be used for decision making purposes. The folling snippet
generates all possible cutsets for the tree represented in Figure
\ref{example-adt}, orders them ascendingly according to probability, and yields
...
itself to analysing those weaknesses and defensive meassures pertaining the
hosting of DNS services in itself, and ignores any attacks that could take place
during transmission.

These threats and the protective meassures regarding the hosting of DNS services
are extracted from \cite[sections 3, 5, and 7]{secdns}. A wider and more
in-depth view can be found in \cite{weaknesses-dns}, \cite{rfc3833} and
\cite{dns-bind-sec}.

\todo{Some quantitative goal to make decisions}
\todo{Mention lack of quantitative data}
\todo{Make up data}
\todo{Prioritise defensive meassures}

\begin{figure}[h]
    \label{dns-transactions}
    \centering
    \includegraphics[height=0.7\paperheight]{dns}
    \caption{}
\end{figure}
